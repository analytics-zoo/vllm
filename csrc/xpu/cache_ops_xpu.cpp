...
template <typename scalar_t>
void call_reshape_and_cache_back_kernel(
  const scalar_t* __restrict__ key,
  const scalar_t* __restrict__ value,
  ...
  const int head_dim,
  const int block_size) {
  int workgroup_size = std::min(512, num_kv_heads * head_dim);

  // Get the queue
  auto& queue = vllm::xpu::vllmGetQueue();
  sycl::range<3> global_range(1, 1, num_tokens);
  ...
  auto cgh = [&](sycl::handler& cgh) {
    cgh.parallel_for(sycl::nd_range<3>(global_range * local_range, local_range),
                     [=](sycl::nd_item<3> item) {
                       ...
                       // Each work-thread should move num_kv_heads * head_dim / work_group_size elements
                       for (int i = local_idx; i < num_kv_heads * head_dim; i += item.get_local_range(2)){
                         // We are handling for element i
                         // [num_token, kv_heads, head_dim]
                         int kv_head_idx = i / head_dim;
                         int kv_haed_offset = i % head_dim;
                        
                        //  int target_key_position = block_idx
                       }
                     });
  };
}
...
void reshape_and_cache_back(
  ...
  // We will need to do a dispatch here
  VLLM_XPU_DISPATCH_FLOATING_TYPES(
    key.scalar_type(), "call_reshape_and_cache_back_kernel", 
    [&]{
      call_reshape_and_cache_back_kernel<scalar_t>(
