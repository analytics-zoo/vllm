...
auto dispatch_context_attention(at::ScalarType it) {
    switch (it) {
        ...
        case at::ScalarType::Half:
            return vllm::context_attention_kernel_kv<sycl::half, GS, HD>;
        default:
...
torch::Tensor context_attention_forward(
    ...
    //         output.stride(1));
    // } else if (head_dim == 64) {
    //     vllm::context_attention_kernel_kv<sycl::half, 32, 64>(
    //         query.data_ptr(), key.data_ptr(), value.data_ptr(),
    //         seq_lens.data_ptr(), attn_scale, output.data_ptr(),
    //         query_start_loc.data_ptr(), batch_size, num_heads, max_input_length,
    //         num_queries_per_kv, query.stride(0), query.stride(1), key.stride(0),
    ...
    // } else {
    //     throw std::runtime_error(
    //         "unsupported head_dim, only 128, and 64 are supported");
    // }

    auto func = [&]() {
        switch (head_dim) {
            case 128:
                return vllm::context_attention_kernel_kv<sycl::half, 32, 128>;
            case 64:
                return vllm::context_attention_kernel_kv<sycl::half, 32, 64>;
            default:
                throw std::runtime_error(
                    "unsupported head_dim, only 128, and 64 are supported");
        }
    }();
    func(query.data_ptr(), key.data_ptr(), value.data_ptr(),
         seq_lens.data_ptr(), attn_scale, output.data_ptr(),
         query_start_loc.data_ptr(), batch_size, num_heads, max_input_length,
         num_queries_per_kv, query.stride(0), query.stride(1), key.stride(0),
         key.stride(1), value.stride(0), value.stride(1), output.stride(0),
