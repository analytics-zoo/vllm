...
inline float qk_dot_(const Vec* q, const Vec* k,
...
#pragma unroll
    for (int mask = THREAD_GROUP_SIZE / 2; mask >= 1; mask /= 2) {
        qk +=
            dpct::permute_sub_group_by_xor(item_ct1.get_sub_group(), qk, mask);
    }
    return qk;
}
...
struct Qk_dot {
    ...
    static inline float dot(const Vec* q, const Vec* k,
                            const sycl::nd_item<3>& item_ct1) {
...
#pragma unroll
    ...
    */
    return dpct::select_from_sub_group(item_ct1.get_sub_group(), sum, 0);
}

// Handle context attention with pure k/v. Currently, we do not have any
// context.
template <typename scalar_t, int GS, int HD>
void context_attention_kernel_kv(
    void* query, void* key, void* value, void* seq_lens, const float scale,
    void* out,  // output
    const void* query_start_loc, const int batch_size, const int num_heads,
    const int max_input_length, const int num_queries_per_group,
    const int query_stride_tokens, const int query_stride_head,
    const int key_stride_tokens, const int key_stride_head,
    const int value_stride_tokens, const int value_stride_head,
    ...
    auto cgf = [&](sycl::handler& handle) {
        handle.parallel_for(
            ...
            [=](sycl::nd_item<3> item) SYCL_ESIMD_KERNEL {
                ...
                const size_t kv_head_idx = head_idx / num_queries_per_group;
                ...
                // TODO: check if this token_idx is correct or not...
                const int32_t* query_loc = (const int32_t*)query_start_loc;
                // There is a possibility that the current token index pass over
                // the seq_len, therefore:
                int32_t token_idx =
                    query_loc[bsz_idx] + std::min(seq_idx, seq_bound - 1);

                const scalar_t* query_head = (const scalar_t*)query +
                                             token_idx * query_stride_tokens +
                                             head_idx * query_stride_head;
                const scalar_t* key_head =
                    (const scalar_t*)key +
                    query_loc[bsz_idx] * key_stride_tokens +
                    kv_head_idx * key_stride_head;
                const scalar_t* value_head =
                    (const scalar_t*)value +
                    query_loc[bsz_idx] * value_stride_tokens +
                    kv_head_idx * value_stride_head;
                // TODO: check out_head validness
                // WARNING: out_head may be out of bound
                scalar_t* out_head =
                    (scalar_t*)out +
                ...
                for (size_t group = 0; group < gid; ++group) {
                    simd<scalar_t, HD> key_row = block_load<scalar_t, HD>(
                        key_head + (group * GS + tid) * key_stride_tokens);
                    slm_block_store(
                        key_slm_offset + tid * HD * sizeof(scalar_t), key_row);
                    simd<scalar_t, HD> value_row = block_load<scalar_t, HD>(
                        value_head + (group * GS + tid) * value_stride_tokens);
                    slm_block_store(
                        value_slm_offset + tid * HD * sizeof(scalar_t),
                        value_row);
                    barrier();
...
#pragma unroll
                    for (size_t r = 0; r < GS; ++r) {
                        ...
                        scalar_t attn = sycl::ext::intel::esimd::detail::sum<
...
#pragma unorll
                ...
                scalar_t softmax =
                    sycl::ext::intel::esimd::detail::sum<scalar_t, scalar_t,
                                                         GS>(softmaxv);

                if (seq_idx < seq_bound) {
                    // key_head has already included the query_loc[bsz_idx]
                    // part, should only included seq_idx part
                    simd<scalar_t, HD> key_row = block_load<scalar_t, HD>(
                        key_head + seq_idx * key_stride_tokens);
                    slm_block_store(
                        key_slm_offset + tid * HD * sizeof(scalar_t), key_row);
                    simd<scalar_t, HD> value_row = block_load<scalar_t, HD>(
                        value_head + seq_idx * value_stride_tokens);
                    slm_block_store(
                        value_slm_offset + tid * HD * sizeof(scalar_t),
                        value_row);
                }
...
void context_attention_kernel_two(
    ...
    auto cgf = [&](sycl::handler& handle) {
        ...
        handle.parallel_for(
            ...
            [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {
                ...
                // TODO: remove this
                const int end_block_idx = MIN(
                    start_block_idx + num_context_blocks, num_context_blocks);
                ...
                constexpr int NUM_VECS_PER_THREAD =
                    NUM_ELEMS_PER_THREAD / VEC_SIZE;
                ...
                const sycl_t* q_ptr =
                    query_ptr + (query_loc_ptr[bsz_idx] + seq_idx) * query_stride_bs + head_idx * HD;
...
#pragma unroll
                for (int i = thread_group_idx; i < NUM_VECS_PER_THREAD;
                    ...
                    q_vecs[thread_group_offset * NUM_VECS_PER_THREAD + i] =
                        *reinterpret_cast<const Q_Vec*>(q_ptr +
                                                        vec_idx * VEC_SIZE);
                }
                // Loaded q_vecs
                item_ct1.barrier(sycl::access::fence_space::local_space);
                        ...
                        for (int j = 0; j < NUM_VECS_PER_THREAD; j++) {
                            const sycl_t* k_ptr =
                                ...
                                physical_block_number *
                                    k_cache_stride_tokens +
                                kv_head_idx * k_cache_stride_head +
                                physical_block_offset * x;

                            const int vec_idx =
                                thread_group_offset + j * THREAD_GROUP_SIZE;
...
#pragma unroll
                for (int mask = WARP_SIZE / 2; mask >= THREAD_GROUP_SIZE;
                    ...
                    */
                    qk_max = sycl::fmax(
                        qk_max, dpct::permute_sub_group_by_xor(
...
#pragma unroll
                ...
                qk_max = dpct::select_from_sub_group(item_ct1.get_sub_group(),
                                                     qk_max, 0);
...
#pragma unroll
                ...
                for (int block_idx = start_block_idx + warp_idx;
                    ...
                    vllm::from_float(logits_vec,
                                     *reinterpret_cast<Float_L_vec*>(
                                         logits + token_idx - start_token_idx));

                    const sycl_t* v_ptr =
                        value_cache_ptr + physical_block_number * v_cache_stride_tokens +
...
#pragma unroll
                    for (int i = 0; i < NUM_ROWS_PER_THREAD; i++) {
                        ...
                        if (row_idx < HD) {
                            const int offset =
                                row_idx * BLOCK_SIZE + physical_block_offset;
                            V_vec v_vec =
                            ...
                            if (block_idx == num_context_blocks - 1) {
                                ...
                                sycl_t* v_vec_ptr =
                                    reinterpret_cast<sycl_t*>(&v_vec);
#pragma unroll
                                for (int j = 0; j < V_VEC_SIZE; j++) {
                                    v_vec_ptr[j] = token_idx + j < context_len
                                                       ? v_vec_ptr[j]
                                                       : zero_value;
                                }
                            }
...
#pragma unroll
                for (int i = 0; i < NUM_ROWS_PER_THREAD; i++) {
                    float acc = accs[i];
#pragma unroll
                    for (int mask = NUM_V_VECS_PER_ROW / 2; mask >= 1;
                         mask /= 2) {
                        /*
...
#pragma unroll
                        for (int i = 0; i < NUM_ROWS_PER_THREAD; i++) {
                            const int row_idx = lane / NUM_V_VECS_PER_ROW +
                                                i * NUM_ROWS_PER_ITER;
                            if (row_idx < HD &&
...
#pragma unroll
                ...
                if (warp_idx == 0) {
                    sycl_t* out_ptr =
                        out_p + (query_loc_ptr[bsz_idx] + seq_idx) * out_stride_tokens + head_idx * out_stride_head;
    ...
    auto cgf = [&](sycl::handler& handle) {
        handle.parallel_for(
            ...
            [=](sycl::nd_item<3> item) SYCL_ESIMD_KERNEL {
                ...
                const int32_t* context_len_pointer =
                ...
                for (int32_t group = 0; group < n; ++group) {
                    ...
                    for (int i = 0; i < HD / x; i++) {
                        // Load 8 elements
                        simd<scalar_t, 8> key_row = block_load<scalar_t, 8>(
...
template <typename scalar_t, typename Q_Vec_t, int HEAD_SIZE, int BLOCK_SIZE,
          int NUM_THREADS, int VEC_SIZE,
...
void paged_attention_kernel(
    ...
    const sycl::nd_item<3>& item_ct1, uint8_t* dpct_local, Q_Vec_t* q_vecs,
    float* red_smem) {
    ...
    // this is just num_context_blocks
    ...
    // has 1, 5, 9, ... th vectors of the query, and so on. NOTE(woosuk):
    // Because q is split from a qkv tensor, it may not be contiguous.
    ...
    */
    item_ct1.barrier(
        sycl::access::fence_space::
            local_space);  // TODO(naed90): possible speedup if this is replaced
                           // with a memory wall right before we use q_vecs
    ...
    for (int block_idx = start_block_idx + warp_idx; block_idx < end_block_idx;
        ...
        // multiplied by large numbers (e.g., kv_block_stride).
        const int64_t physical_block_number =
            static_cast<int64_t>(block_table[block_idx]);
...
#pragma unroll
            ...
            // group. Q_Vec_t q_vec_[NUM_VECS_PER_THREAD] = q_vecs +
            // thread_group_offset * THREAD_GROUP_SIZE;
            float qk =
                scale * Qk_dot<scalar_t, THREAD_GROUP_SIZE>::template dot<
                            Q_Vec_t, NUM_VECS_PER_THREAD>(
...
#pragma unroll
    ...
    for (int block_idx = start_block_idx + warp_idx; block_idx < end_block_idx;
        ...
        L_vec logits_vec;
        vllm::from_float(logits_vec, *reinterpret_cast<Float_L_vec*>(
                                         logits + token_idx - start_token_idx));
...
#pragma unroll
        for (int i = 0; i < NUM_ROWS_PER_THREAD; i++) {
            ...
            if (row_idx < HEAD_SIZE) {
                ...
                if (block_idx == num_context_blocks - 1) {
                    // NOTE(woosuk): When v_vec contains the tokens that are out
                    // of the context, we should explicitly zero out the values
                    // since they may contain NaNs. See
                    // https://github.com/vllm-project/vllm/issues/641#issuecomment-1682544472
...
#pragma unroll
        for (int mask = NUM_V_VECS_PER_ROW / 2; mask >= 1; mask /= 2) {
            /*
            DPCT1096:41: The right-most dimension of the work-group used in the
            ...
            "32".
            */
            acc += dpct::permute_sub_group_by_xor(item_ct1.get_sub_group(), acc,
                                                  mask);
        }
        accs[i] = acc;
    }
...
void paged_attention_v1_kernel(
    ...
    const sycl::nd_item<3>& item_ct1, uint8_t* dpct_local, Q_Vec_t* q_vecs,
    float* red_smem) {
    paged_attention_kernel<scalar_t, Q_Vec_t, HEAD_SIZE, BLOCK_SIZE,
                           NUM_THREADS, VEC_SIZE>(
        /* exp_sums */ nullptr,
        /* max_logits */ nullptr, out, q, k_cache, v_cache, num_kv_heads, scale,
    ...
    queue.submit([&](sycl::handler& cgh) {                                    \
        ...
        cgh.parallel_for(                                                     \
            ...
            [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(      \
                32)]] {                                                       \
                paged_attention_v1_kernel<sycl_t, Q_Vec, HEAD_SIZE,           \
                                          BLOCK_SIZE, NUM_THREADS, VEC_SIZE>( \
                    out_ptr_ct0, query_ptr_ct1, key_cache_ptr_ct2,            \
                    value_cache_ptr_ct3, num_kv_heads, scale_ct5,             \
...
void paged_attention_xpu_v1_impl_launcher(
    ...
    switch (head_size) {
        ...
        // head sizes that we use in the model. However, we can easily extend
        // this to support any head size which is a multiple of 16.
        case 64:
...
void paged_attention_v2_reduce_kernel(
    ...
    const int* __restrict__ context_lens,  // [num_seqs]
    const int max_num_partitions, const sycl::nd_item<3>& item_ct1,
    uint8_t* dpct_local, float* red_smem) {
    const int num_heads = item_ct1.get_group_range(2);
    const int head_idx = item_ct1.get_group(2);
    const int seq_idx = item_ct1.get_group(1);
    const int context_len = context_lens[seq_idx];
...
#pragma unroll
    for (int mask = WARP_SIZE / 2; mask >= 1; mask /= 2) {
        ...
        */
        max_logit = sycl::fmax(
            max_logit, dpct::permute_sub_group_by_xor(item_ct1.get_sub_group(),
                                                      max_logit, mask));
    }
    if (lane == 0) {
...
#pragma unroll
    ...
    */
    max_logit =
        dpct::select_from_sub_group(item_ct1.get_sub_group(), max_logit, 0);
...
void paged_attention_v2_kernel(
    ...
    const float* __restrict__ alibi_slopes,  // [num_heads]
    const int q_stride, const int kv_block_stride, const int kv_head_stride,
    // [1, num_seqs, num_heads], [1, 1, WORKGROUP_SIZE]
    const sycl::nd_item<3>& item_ct1, uint8_t* dpct_local, Q_Vec_t* q_vecs,
    float* red_smem) {
    paged_attention_kernel<scalar_t, Q_Vec_t, HEAD_SIZE, BLOCK_SIZE,
                           NUM_THREADS, VEC_SIZE, PARTITION_SIZE>(
        ...
        cgh.parallel_for(                                                      \
            ...
            [=](sycl::nd_item<3> item_ct1)                                     \
                [[intel::reqd_sub_group_size(32)]] {                           \
                    vllm::paged_attention_v2_kernel<sycl_t, Q_Vec, HEAD_SIZE,  \
                                                    BLOCK_SIZE, NUM_THREADS,   \
                                                    VEC_SIZE, PARTITION_SIZE>( \
                        exp_sums_ptr_ct0, max_logits_ptr_ct1, tmp_out_ptr_ct2, \
                        query_ptr_ct3, key_cache_ptr_ct4, value_cache_ptr_ct5, \
                        num_kv_heads, scale_ct7, block_tables_ptr_ct8,         \
                        context_lens_ptr_ct9, max_num_blocks_per_seq_ct10,     \
                        alibi_slopes_ptr_ct11, q_stride_ct12,                  \
                        kv_block_stride_ct13, kv_head_stride_ct14, item_ct1,   \
...
template <
    typename T, int BLOCK_SIZE,
    int NUM_THREADS = 128,  // NUM_THREADS denote the threads in each thread
                            // block
    int PARTITION_SIZE =
        512>  // In declaration, it says that this indicates the size of TP size
void paged_attention_v2_launcher(
    ...
    int num_heads = query.size(1);
    int head_size = query.size(2);
    // [num_seqs, max_num_blocks_per_seq, but it may contain padding_ids]
    int max_num_blocks_per_seq = block_tables.size(1);
    int q_stride = query.stride(0);
    int kv_block_stride = key_cache.stride(
        0);  //(num_blocks, num_kv_heads, head_size // x, block_size, x)
    int kv_head_stride = key_cache.stride(1);

    // THEAD_GROUP_SIZE is 2
    constexpr int THREAD_GROUP_SIZE = MAX(WARP_SIZE / BLOCK_SIZE, 1);
    assert(head_size % THREAD_GROUP_SIZE == 0);
    // VEC_SIZE is 16 / 2 * 2 = 4
    constexpr int VEC_SIZE = MAX(16 / (THREAD_GROUP_SIZE * sizeof(T)), 1);
    using sycl_t = vllm::xpu::SyclTypeTrait<T>::Type;
    using Q_Vec = typename Vec<sycl_t, VEC_SIZE>::Type;

    // num_vecs_per_thread = 128 / 2 / 4 = 16
    int num_vecs_per_thread = head_size / THREAD_GROUP_SIZE / VEC_SIZE;
    assert(head_size % THREAD_GROUP_SIZE == 0);
    ...
    // NUM_WRAPS = 128 / 32 = 4 wraps
    constexpr int NUM_WARPS = NUM_THREADS / WARP_SIZE;

    // Just assume this is 1
    ...
    // Assume this is just 1, num_seqs, num_heads
    sycl::range<3> grid(max_num_partitions, num_seqs, num_heads);
    int shared_mem_size = std::max(logits_size, outputs_size);
    // For paged attention v2 reduce kernel.

    // What is this reduce_grid
...
template <const int GS, const int HD>
auto dispatch_context_attention(at::ScalarType it) {
    switch (it) {
        case at::ScalarType::Float:
            throw std::runtime_error("Unsupported dtype float32");
        case at::ScalarType::Half:
            return vllm::context_attention_kernel<at::ScalarType::Half, GS, HD>;
        default:
            throw std::runtime_error(
                "unsupported dtype, only fp16 are supported");
    }
}
...
torch::Tensor context_attention_forward(
    ...
    torch::Tensor seq_lens, torch::Tensor context_lens, int max_input_length, int max_context_length) {
    ...
    if (key_dimension == 3) {
        assert(num_tokens == key.size(0) && key.size(0) == value.size(0));
        int key_stride_token = key.stride(0);
        int key_stride_head = key.stride(1);
        // Probably the same with query/value
        int key_stride_dim = key.stride(2);
        int value_stride_bs = value.stride(0);
        int value_stride_head = value.stride(1);
        int value_stride_dim = value.stride(2);
        vllm::context_attention_kernel_kv<sycl::half, 32, 128>(
            query.data_ptr(), key.data_ptr(), value.data_ptr(),
            seq_lens.data_ptr(), attn_scale, output.data_ptr(),
            query_start_loc.data_ptr(), batch_size, num_heads, max_input_length,
            num_queries_per_kv, query.stride(0), query.stride(1), key.stride(0),
            key.stride(1), value.stride(0), value.stride(1), output.stride(0),
            output.stride(1));
        return output;
    } else {
        ...
        vllm::context_attention_kernel_two<sycl::half, 32, 128>(
            query.data_ptr(), key.data_ptr(), value.data_ptr(),
            ...
            max_input_length, batch_size, num_heads, query.size(0), max_context_length);
