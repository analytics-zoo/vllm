...
void mha_kernel(
    ...
    const void* context_lens, // [num_seqs]
    void * output,
    const size_t  query_bsz_stride,
    const size_t  query_head_stride,
    const size_t  key_token_stride,
    const size_t  key_head_stride,
    const size_t  key_block_stride,
    const size_t  value_token_stride,
    const size_t  value_head_stride,
    const size_t  value_block_stride,
    const size_t  block_table_stride_batch,
    const size_t  output_bsz_stride,
    const size_t  output_head_stride,
    const size_t  output_seq_stride,
    const float scale,
    ...
    const int group_num = num_heads / num_kv_heads;
    ...
    auto cgf = [&](sycl::handler& handle) {
        handle.parallel_for(
            ...
            [=](sycl::nd_item<3> item) SYCL_ESIMD_KERNEL {
                ...
                const int bsz_idx = item.get_global_id(0);
                const int head_idx = item.get_global_id(1);
                const int kv_head_idx = head_idx / group_num;
                // const int seq_idx = item.get_group(2);
                ...
                const int sub_rows = context_length / GS;
                const int rem_rows = context_length % GS;

                simd<IT, HD> query_row = block_load<IT, HD>(query_head) * attn_scale;

                int start_row = sub_rows * tid + std::min(tid, rem_rows);
                int end_row = start_row + sub_rows + (tid < rem_rows);
    ...
    utils::submit_kernel(cgf, device, "mha kernel");
}

template<const int GS, const int HD>
auto dispatch_mha_kernel(AT it) {
    switch (it) {
        case AT::Float: return mha_kernel<float, GS, HD>;
        case AT::Half: return mha_kernel<fp16, GS, HD>;
        default: throw std::runtime_error("unsupported dtype, only fp32 and fp16 are supported");
    }
}
...
void paged_attention_mha(
    ...
    auto func = [&](){
        switch (head_dim) {
            case 128: return dispatch_mha_kernel<GS, 128>(query.scalar_type());
            case 96: return dispatch_mha_kernel<GS, 96>(query.scalar_type());
            case 80: return dispatch_mha_kernel<GS, 80>(query.scalar_type());
            case 64: return dispatch_mha_kernel<GS, 64>(query.scalar_type());
            default: throw std::runtime_error("unsupported head_dim, only 128, 96, 80 and 64 are supported");
        }
    }();
    ...
    func(
        query.data_ptr(), key_cache.data_ptr(), value_cache.data_ptr(),
        block_tables.data_ptr(), context_lens.data_ptr(), output.data_ptr(),
        ...
        scale, block_size, bsz, num_heads, num_kv_heads, max_seq_len, max_context_len,
        query.device()
    );
}